...
public class OrdersController {
	...
	public ModelAndView ShippingAddress(@ModelAttribute ShippingAddress shippingaddress,HttpSession session,HttpServletRequest request)
	{
		...
		orderDAO.insertShippingAddress(shippingaddress);
	...
	public ModelAndView BillingAddress(@ModelAttribute BillingAddress billingaddress,HttpSession session,HttpServletRequest request)
	{
		...
		orderDAO.insertBillingAddress(billingaddress);
	...
	public ModelAndView insertorder(@ModelAttribute Orders order,HttpServletRequest request,HttpSession session)
	{
		...
		session=request.getSession(false);
		String username=(String)session.getAttribute("username");
		shippingaddress=orderDAO.getShippingByUsername(username);
		billingaddress=orderDAO.getBillingByUsername(username);
		cart=orderDAO.getCartByUsername(username);
		...
		List<Orders> orders=orderDAO.orderList(username);
		mv.addObject("orders",orders);
		List<ShippingAddress> shipping=orderDAO.shippingAddressList(username);
		mv.addObject("shipping", shipping);
		List<BillingAddress> billing=orderDAO.billingAddressList(username);
